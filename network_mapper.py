...
c1, c2 = st.columns(2)
with c1:
    loc_file = st.file_uploader("Locations CSV (CITY, STATE, LAT, LONG)", type=["csv"], key="nm_locs")
with c2:
    cust_file = st.file_uploader("Customers CSV (LAT, LONG)", type=["csv"], key="nm_custs")
...
selected_names = st.multiselect("Choose warehouses to include",
...
st.caption("Tip: Place this file in `pages/` to expose it as a separate page.")
